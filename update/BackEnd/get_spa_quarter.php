...
// Câu truy vấn SELECT tổng hợp tổng số đơn đặt và tổng giá tiền theo từng dịch vụ và từng quý
$sql = "SELECT 
            serviceName, 
            YEAR(date) AS year, 
            CEIL(MONTH(date) / 3) AS quarter,  -- Tính quý từ tháng
            COUNT(*) AS total_orders, 
            SUM(servicePrice) AS total_revenue
        FROM appointments 
        WHERE payment_status = 'Hoàn thành'
        GROUP BY serviceName, YEAR(date), quarter
        ORDER BY year, quarter, serviceName ASC";
